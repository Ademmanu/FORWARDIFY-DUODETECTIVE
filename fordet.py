...
if not DATABASE_URL:
    logger.error("❌ DATABASE_URL environment variable must be set for PostgreSQL")
    sys.exit(1)
...
class Database:
    ...
    def _load_caches(self):
        ...
        try:
            ...
            logger.exception(f"Error loading caches: {e}")
    ...
    def get_connection(self):
        ...
        try:
            ...
            logger.exception(f"Failed to create DB connection: {e}")
            raise
    ...
    def init_db(self):
        ...
        with self._conn_init_lock:
            ...
            with conn.cursor() as cur:
                ...
                """)
                cur.execute("""
                    CREATE INDEX IF NOT EXISTS idx_forwarding_tasks_user_active 
                    ON forwarding_tasks(user_id, is_active)
                """)
                cur.execute("""
                    CREATE INDEX IF NOT EXISTS idx_monitoring_tasks_user_active 
    ...
    def get_user(self, user_id: int) -> Optional[Dict]:
        ...
        try:
            ...
            logger.exception(f"Error in get_user for {user_id}: {e}")
            return None
    ...
    def save_user(self, user_id: int, phone: Optional[str] = None, name: Optional[str] = None,
        ...
        try:
            ...
            logger.exception(f"Error in save_user for {user_id}: {e}")
            conn.rollback()
            raise
    ...
    def add_forwarding_task(self, user_id: int, label: str, source_ids: List[int], 
        ...
        try:
            ...
            logger.exception(f"Error in add_forwarding_task for {user_id}: {e}")
            conn.rollback()
            raise
    ...
    def update_task_filters(self, user_id: int, label: str, filters: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.exception(f"Error in update_task_filters for {user_id}, task {label}: {e}")
            conn.rollback()
            raise
    
    def remove_forwarding_task(self, user_id: int, label: str) -> bool:
        """Remove forwarding task"""
        try:
            conn = self.get_connection()
            ...
            logger.exception(f"Error in remove_forwarding_task for {user_id}: {e}")
            conn.rollback()
            raise
    
    def get_user_forwarding_tasks(self, user_id: int) -> List[Dict]:
        """Get all forwarding tasks for a user"""
        ...
        try:
            ...
            logger.exception(f"Error in get_user_forwarding_tasks for {user_id}: {e}")
            return []
    ...
    def get_all_active_forwarding_tasks(self) -> List[Dict]:
        ...
        try:
            ...
            logger.exception(f"Error in get_all_active_forwarding_tasks: {e}")
            raise
    ...
    def add_monitoring_task(self, user_id: int, label: str, chat_ids: List[int],
        ...
        try:
            ...
            logger.exception(f"Error in add_monitoring_task for {user_id}: {e}")
            conn.rollback()
            return False
    
    def update_task_settings(self, user_id: int, label: str, settings: Dict[str, Any]) -> bool:
        """Update monitoring task settings"""
        try:
            conn = self.get_connection()
            ...
            logger.exception(f"Error in update_task_settings for {user_id}, task {label}: {e}")
            conn.rollback()
            return False
    ...
    def remove_monitoring_task(self, user_id: int, label: str) -> bool:
        ...
        try:
            ...
            logger.exception(f"Error in remove_monitoring_task for {user_id}: {e}")
            conn.rollback()
            return False
    ...
    def get_user_monitoring_tasks(self, user_id: int) -> List[Dict]:
        ...
        try:
            ...
            logger.exception(f"Error in get_user_monitoring_tasks for {user_id}: {e}")
            return []
    ...
    def get_all_active_monitoring_tasks(self) -> List[Dict]:
        ...
        try:
            ...
            logger.exception(f"Error in get_all_active_monitoring_tasks: {e}")
            return []
    ...
    def is_user_allowed(self, user_id: int) -> bool:
        ...
        try:
            ...
            logger.exception(f"Error checking is_user_allowed for {user_id}: {e}")
            return False
    ...
    def is_user_admin(self, user_id: int) -> bool:
        ...
        try:
            ...
            logger.exception(f"Error checking is_user_admin for {user_id}: {e}")
            return False
    ...
    def add_allowed_user(self, user_id: int, username: Optional[str] = None,
        ...
        try:
            ...
            logger.exception(f"Error in add_allowed_user for {user_id}: {e}")
            conn.rollback()
            return False
    ...
    def remove_allowed_user(self, user_id: int) -> bool:
        ...
        try:
            ...
            logger.exception(f"Error in remove_allowed_user for {user_id}: {e}")
            conn.rollback()
            return False
    ...
    def get_all_allowed_users(self) -> List[Dict]:
        ...
        try:
            ...
            logger.exception(f"Error in get_all_allowed_users: {e}")
            return []
    ...
    def get_logged_in_users(self, limit: Optional[int] = None) -> List[Dict]:
        ...
        try:
            ...
            logger.exception(f"Error fetching logged-in users: {e}")
            raise
    ...
    def get_all_string_sessions(self) -> List[Dict]:
        ...
        try:
            ...
            logger.exception(f"Error in get_all_string_sessions: {e}")
            raise
    ...
    def get_db_status(self) -> Dict:
        ...
        try:
            ...
            logger.exception(f"Error getting DB status: {e}")
            status["error"] = str(e)
...
async def db_call(func, *args, **kwargs):
    """Execute database function in thread pool"""
